...
local function set_group_photo(msg, success, result)
  ...
  if success then
    ...
    send_large_msg(receiver, 'مشکلی در زمان نصب پیش آمد دوباره امتحان کنید!', ok_cb, false)
  end
end
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'setgroup' then
      ...
      if matches[2] == 'realm' then
        ...
        return 'مدل گروه به ریلم تغییر کرد'
      end
      if matches[2] == 'support' then
        ...
        return 'مدل گروه به پشتیبانی تغییر کرد'
      end
      if matches[2] == 'feedback' then
        ...
        return 'مدل گروه به نظردهی تغییر کرد'
      end
      if matches[2] == 'vip' then
        ...
        return 'مدل گروه به مخصوص تغییر کرد'
      end
      if matches[2] == 'normal' then
        if groupmodel == 'normal' then
          return 'مدل گروه به معمولی تغییر کرد'
        else
          data[tostring(msg.to.id)]['settings']['groupmodel'] = 'normal'
          save_data(_config.moderation.data, data)
          return 'مدل گروه به معمولی تغییر یافت'
        end
      end
    end
    ...
    if matches[1] == 'دارنده' or matches[1] == 'setleader' and matches[2] then
      ...
      local text = matches[2].." صاحب گروه است"
      return text
    end
