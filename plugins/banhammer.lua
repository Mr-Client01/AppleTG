...
--vardump(extra)
      ...
      local receiver = "chat#id"..chat_id
       if get_cmd == "kick" then
         ...
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            return send_large_msg(receiver, "شما نمیتوانید ادمین هارا اخراج کنید")
         end
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از تمام گروه های من بن شد')
...
local function run(msg, matches)
 if matches[1]:lower() == 'id' then
    ...
    elseif matches[1]:lower() == 'id' then
      local name = user_print_name(msg.from)
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "آیدی گروه " ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id.."\nاز این آیدی میتوانید برای عضویت در بعدا بهره ببرید\nبرای مثال\n!join "..msg.to.id.."
    end
  end
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'کاربر ['..user_id..' ] از تمام گروه های من بن شد'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
